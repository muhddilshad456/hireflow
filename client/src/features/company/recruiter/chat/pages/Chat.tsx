...
} from "../../../../shared/services/messageService"; // adjust path to your actual service file
...
type SenderRole = "company_recruiter" | "user";
...
function Avatar({
  ...
  return (
    <div
      style={style}
      className="rounded-full flex-shrink-0 flex items-center justify-center text-white text-xs font-semibold bg-blue-500"
    >
      {initials}
    </div>
  );
        ...
        <div
          className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
            isOwn
              ...
              : "bg-blue-50 text-slate-800 rounded-bl-sm border border-blue-100"
          }`}
        >
...
function MessageInput({
  ...
  return (
    <div className="border-t border-blue-100 p-3">
      <div className="flex items-center gap-2 bg-blue-50/60 rounded-xl px-3 py-2">
        <button
...
export function ChatPage() {
  ...
  // Recruiter can always initiate, so no "locked" state to worry about here —
  // only genuine failures (network, not found, not the job owner) hit "error".
  useEffect(() => {
    if (!applicationId) return;
    if (initializedForRef.current === applicationId) return; // StrictMode double-invoke guard
    initializedForRef.current = applicationId;

    const init = async () => {
      try {
        const result = await createOrGetConversation(applicationId);
        console.log("result of single conversation fetch : ", result);
  ...
  return (
    <div className="h-full flex flex-col p-4 sm:p-6">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-white border border-blue-100 rounded-xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-blue-100 bg-blue-50/40">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Candidate · {conversation.jobTitle}
            </p>
          </div>
        </div>

        {/* Messages */}
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 bg-blue-50/10">
          {!conversation.isActive && (
            <div className="text-center text-[11px] text-slate-400 bg-blue-50 rounded-full py-1.5 px-3 w-fit mx-auto">
